...
SELECT  dv.DATA.customers.custId CUST_ID,
    ...
    FROM SALES_HISTORY_DUALITY_VIEW dv
    WHERE dv.DATA.customers.custId = 9696
;
/* 
...
-- 
CREATE FORCE EDITIONABLE JSON RELATIONAL DUALITY VIEW JSON_ORDERS.SALES_HISTORY_DUALITY_VIEW_ALL
 AS SELECT JSON {
        'quantitySold' : s.QUANTITY_SOLD,
        'amountSold' : s.AMOUNT_SOLD,
        '_id' : s.SALES_ID,
        'products' : 
            ( SELECT JSON {
                'prodId' : p1.PROD_ID,
                'prodName' : p1.PROD_NAME
            }
            FROM JSON_ORDERS.PRODUCTS p1
             WHERE  p1.PROD_ID = s.PROD_ID 
         ),
        'customers' : 
            ( SELECT JSON {
                'custId' : c.CUST_ID,
            ...
            FROM JSON_ORDERS.CUSTOMERS c
             WHERE  c.CUST_ID = s.CUST_ID 
         ),
        'channels' : 
            ( SELECT JSON {
                'channelId' : c1.CHANNEL_ID,
                'channelDesc' : c1.CHANNEL_DESC
            }
            FROM JSON_ORDERS.CHANNELS c1
             WHERE  c1.CHANNEL_ID = s.CHANNEL_ID 
         )
    }
    FROM JSON_ORDERS.SALES s
;